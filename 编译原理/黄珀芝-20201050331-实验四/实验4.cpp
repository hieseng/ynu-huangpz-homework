...
int main()
{
    ...
    {
        printf("List intermediate code(列表中间代码)?(Y/N)"); // 是否输出中间代码
        scanf("%s", filename);
        ...
        printf("Optimize the intermediate code(优化中间代码)?(Y/N)"); //是否进行优化并输出优化的中间代码
        scanf("%s", filename);
        opt = (filename[0] == 'y' || filename[0] == 'Y');

        printf("Run object code(运行目标代码)?(Y/N)"); // 是否运行虚拟机代码
        scanf("%s", filename);
